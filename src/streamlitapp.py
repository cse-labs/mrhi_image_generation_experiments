...
from generate_event_handler import generate_nutrient_panel_handler
load_dotenv()
...
select_option = option_menu(menu_title=None, options=['On-pack', 'Off-pack', 'Nutrient Panel'], orientation='horizontal')
...
"""'''
    ...
    original_image: FileUploaderMixin = col1.file_uploader('Upload the base front pack image for MRHI generation')
    original_mrhi_image = col2.file_uploader('Upload the human generated MRHI image for evaluation')
    text_input = st.text_input('Bottom Lozenges Text', placeholder='Enter information about the product here')
    generate_button = st.button('Generate')
    if original_image is None and generate_button:
        st.warning('Please upload the original image')
    if generate_button:
        (generated_image, validation_results, evaluated_result) = generate_event_handler.on_pack_generate_clicked(original_image, original_mrhi_image, text_input)
        (col1, col2) = st.columns(2)
        col1.image(original_image, caption='Original Image', use_column_width=True)
        col2.image(generated_image, caption='Generated MRHI Image', use_column_width=True)
...
"""'''
    ...
    (col1, col2) = st.columns(2)
    original_image = col1.file_uploader('Upload the base front pack image for MRHI generation')
    original_mrhi_image = col2.file_uploader('Upload the human generated MRHI image for evaluation')
    (col1, col2) = st.columns(2)
    Snacks_name = col1.text_input('Product name', placeholder='Enter the Product name here')
    Snacks_weight = col2.text_input('Grammage', placeholder='Enter the Product Grammage here')
    generate_button = st.button('Generate')
    if original_image is None and generate_button:
        st.warning('Please upload the original image')
    elif original_image and generate_button:
        (generated_image, evaluation_metrics) = generate_event_handler.off_pack_generate_clicked(original_image=original_image, original_mrhi_image=original_mrhi_image, left_text=Snacks_name, right_text=Snacks_weight)
        (col1, col2) = st.columns(2)
        col1.image(original_image, caption='Original Image', use_column_width=True)
        col2.image(generated_image, caption='Generated MRHI Image', use_column_width=True)

def display_nutrient_panel():
    '''"""
This function displays the nutrient panel of an ingredient. It first prompts the user to upload an image of the ingredient. 
Once the image is uploaded and the 'Generate' button is clicked, the function calls the 'generate_nutrient_panel_handler' 
function to extract the nutrient panel from the uploaded image. The original image and the extracted nutrient panel are 
then displayed side by side. If the 'Generate' button is clicked without uploading an image, a warning message is displayed.
...
None
"""'''
    st.header('Ingredient Generation')
    original_image = st.file_uploader('Upload the image')
    generate_button = st.button('Generate')
    if original_image is None and generate_button:
        st.warning('Please upload the image')
    elif original_image and generate_button:
        extracted_ingredient_panel = generate_nutrient_panel_handler(original_image, None)
        (col1, col2) = st.columns(2)
        col1.image(original_image, caption='Original Image', use_column_width=True)
        col2.image(extracted_ingredient_panel, caption='Extacted Ingredient Panel', use_column_width=True)
if select_option == 'On-pack':
    display_on_pack()
elif select_option == 'Off-pack':
    display_off_pack()
elif select_option == 'Nutrient Panel':
    display_nutrient_panel()